  ...
  onSubmit = () => {
    ...
    return fetch('https://api.cloudinary.com/v1_1/kusinahanglan/image/upload', options)
      ...
      .then(res => {
        ...
        axios.post('http://localhost:5000/carousel_header/add_carousel_header', exercise)
        .then(res =>{
          cogoToast.success('Please wait');
            if(res.data == 'Added!'){
...
onDelete = (id,title) => {
    ...
    confirmAlert({
        ...
        buttons: [
          {
            label: 'Yes',
            onClick: () =>  axios.post('http://localhost:5000/highlight/Highlight_Delete', exercise)
            .then(res =>{
...
<br />
          ...
          <TextField id="outlined-basic" label="Subtitles" variant="outlined" onChange={this.onChangedescription}/>
          <br />
          <label for="file">Image(50px x 43px):</label><br />
          <input type="file"/>
