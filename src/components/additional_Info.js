...
console.log('files[0] :', files[0])
    if(files[0] == undefined){
        ...
        const exercise = {
            title:this.state.Header_Text,
            subtitles:  this.state.Header_Subtext ,
            link: this.state.link ,
            id: this.state.video_area_id,
           }
   
           axios.post('http://localhost:5000/video_area/Update_NoBackground', exercise)
             .then(res =>
              window.location.reload()
             )
        }else{
      ...
      .then(res => {
        ...
        const exercise = {
          ...
          background_image:  res.secure_url,    
           ...
           axios.post('http://localhost:5000/video_area/Update_Background', exercise)
             .then(res =>
  ...
  render() {
    ...
    return (
     ...
     <li onClick={()=> this.getInfoID(info.partition, info.expiration)} key={i} className="treeview active">
        ...
        <label for="title">Background Image:</label> <br />
                    <input type="file" id="header_image"/>
                    <img src={this.state.background_image===""?'https://artsmidnorthcoast.com/wp-content/uploads/2014/05/no-image-available-icon-6.png':this.state.background_image} className="displayed-image" width={250} height={250} />
<br />
